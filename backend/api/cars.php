...
switch ($method) {
    ...
    case 'POST':
        ...
        if (
            ...
            $car->km = $data->km ?? 0;
            $car->transmission = $data->transmission ?? 'automatico';
            $car->fuel = $data->fuel ?? 'gasolina';
            $car->type = $data->type ?? 'sedan';
            $car->color = $data->color ?? 'blanco';
            $car->city = $data->city ?? '';
            $car->status = $data->status ?? 'disponible';
            $car->featured = isset($data->featured) ? $data->featured : false;
            $car->home_section = $data->home_section ?? null;
            ...
            echo json_encode(array(
                "success" => false,
                "message" => "Unable to create car. Data is incomplete."
            ));
        }
        break;
    ...
    case 'PUT':
        ...
        if (!empty($_GET['id'])) {
            ...
            if (
                ...
                echo json_encode(array(
                    "success" => false,
                    "message" => "Unable to update car. Data is incomplete."
                ));
            }
        } else {
            echo json_encode(array(
